...
class TranscriptDAO:
    ...
    def __init__(
        ...
        limit: Optional[int],
        messages: Optional[List[hikari.Message]],
        pytz_timezone,
        military_time: bool,
        fancy_times: bool,
    ...
    async def export_transcript(self, message_html: str, meta_data: str):
        ...
        generated_icon_url = guild.make_icon_url()
        guild_icon = generated_icon_url if generated_icon_url else DiscordUtils.default_avatar
        ...
        raw_channel_topic = (
            self.channel.topic if isinstance(self.channel, hikari.GuildTextChannel) and self.channel.topic else ""
        )
...
class Transcript(TranscriptDAO):
    async def export(self):
        if not self.messages:
            self.messages = [message async for message in self.channel.fetch_history(
                before=self.before,
                after=self.after,
            )]
